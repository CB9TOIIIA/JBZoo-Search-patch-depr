...
if (!$this->app->jbcache->start()) {
    ...
    if ($this->items) {
        ...
        echo '<p class="countresultcart">' . JText::_('JBZOO_FILTER_TOTAL_RESULT') . ': <span class="cartmskcountresult">' . $this->itemsCount . '</span></p>';
      // $searchCountCart =  $this->itemsCount;
      // echo $searchCountCart;
        echo "<script>jQuery(function($){ if ($('.cartmskcountresult').text() === '1') {  location.assign($('.cartlink a').attr('href'));} });</script>";
